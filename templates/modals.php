            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="email" name="user_email" <?php if(!empty($_COOKIE['useremail'])){ echo 'value="'.$_COOKIE['useremail'].'"'; } ?> class="form-control" required>
                    </div>
                    <div class="mb-3">
                        <label for="user_pass" class="form-label">Password</label>
                        <input type="password" name="user_pass" <?php if(!empty($_COOKIE['userpass'])){ echo 'value="'.$_COOKIE['userpass'].'"'; } ?> class="form-control" required>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="1" name="remember_me" <?php if(!empty($_COOKIE['remember_me'])){ echo 'checked'; } ?>>
                        <label class="form-check-label" for="remember_me">
                            Remember me/keep me loggedin
                        </label>
                    </div>
                </div>

                <div class="modal-footer">
                    <button type="submit" name="user_login_req" class="btn btn-outline-success">Login</button>
                </div>
            </form>
        </div>
    </div>
